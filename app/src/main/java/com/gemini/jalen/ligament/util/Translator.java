...
public class Translator {
    ...
    protected Translator(boolean hasChinaCollator) {
        ...
        synchronized (Translator.class) {
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINA)
                    ...
                    sInstance = new Translator(true);
                    return sInstance;
                }
            }
            sInstance = new Translator(false);
            return sInstance;
        }
    }
    ...
    private Token getToken(char character) {
        ...
        if (StringUtil.isEmpty(token.target)) {
            token.type = Token.UNKNOWN;
    ...
    /**
     * Convert the input to notification_def array of tokens. The sequence of ASCII or Unknown
     * characters without space will be put into notification_def Token, One Hanzi character
     * which has pinyin will be treated as notification_def Token. If these is no China
     * collator, the empty token array is returned.
     */
    public ArrayList<Token> get(final String input) {
        ArrayList<Token> tokens = new ArrayList<Token>();
        if (!mHasChinaCollator || StringUtil.isEmpty(input)) {
            // return empty tokens.
            return tokens;
        }
        ...
        // Go through the input, create notification_def new token when
        // notification_def. Token type changed
